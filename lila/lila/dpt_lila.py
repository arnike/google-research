...
class RFFEncoder(nn.Module):
    ...
    """
    def __init__(self, scale_flow: float = 1.0, scale_depth: float = 1.0):
        """
        Args:
    ...
    def seed(self, x: torch.Tensor, y: torch.Tensor):
        if x.ndim == 4:
            x = self.tokenise(x)
            y = self.tokenise(y)
    ...
    def renorm(self, x: torch.Tensor, y: torch.Tensor):
        return (x - self.x_mean) / (1e-3 + self.x_std), \
                (y - self.y_mean) / (1e-3 + self.y_std)
    
    def forward_bnd(self, x: torch.Tensor, y: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
        """
    ...
    def forward_bchw(self, x: torch.Tensor, y: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
        b,c,h,w = x.shape
...
def load_lila_weights(
    ...
    checkpoints_dir: Optional[Union[str, os.PathLike]] = None,
...
def load_encoder(encoder_name: str, checkpoints_dir=None) -> nn.Module:
    model_config = {
...
def build_lila_model(encoder_name: str, checkpoints_dir=None) -> LILA:
...
class TrainLILA(LILA):
    ...
    def random_subset(self, x):
        ...
        ids_keep = ids_shuffle[..., :self.cfg.num_fts]
