...
def compute_psnr(im1, im2, maxval=1.):
...
def _computeMetricsV2(confusion, unseen_indices=None):
    ...
    :param unseen_indices: (Optional) list or array of indices corresponding to unseen classes
    :return: 
...
class CocoEval(BaseMetric):
    ...
    def check_with_dataset(self, dataset):
        assert dataset.num_classes() == self.label_count, \
            f"Dataset has {dataset.num_classes()} classes; defined {self.label_count}"
    ...
    def dict(self):
        [miou, fwiou, macc, pacc, ious, maccs] = _computeMetrics(self.confusion)
        metrics = {"mIoU": miou.item(), "fwIoU": fwiou.item(), "mAcc": macc.item(), "pAcc": pacc.item()}
...
class CocoEvalZeroShot(CocoEval):
    ...
    def dict(self, unseen_indices=None):
        ...
        if not unseen_indices is None:
            ...
            metrics.update({"mIoU_seen": miou_s.item(), "fwIoU_seen": fwiou_s.item(), "mAcc_seen": macc_s.item(),
                            "mIoU_unseen": miou_u.item(), "fwIoU_unseen": fwiou_u.item(), "mAcc_unseen": macc_u.item(),
...
class DepthEval(BaseMetric):
    ...
    def add_prediction(self, depth_pr, depth_gt):
        ...
        thresh = torch.maximum(depth_gt / depth_pr.clamp(min=1e-9), depth_pr / depth_gt.clamp(min=1e-9))
