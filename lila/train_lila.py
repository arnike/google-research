...
from utils.vis import DisplayConf, DisplayDepth, DisplayFlow, DisplaySeg, convert2pca
...
class LinProbe(nn.Module):
    ...
    def forward(self, feats, scale_to):
        ...
        return F.interpolate(logits, scale_to, mode="bilinear", align_corners=False)


class EvalVOS(nn.Module):
    """Fit a lightweight first-frame probe for DAVIS-style validation."""
...
class TrainModel(L.LightningModule):
    def __init__(self, cfg):
        ...
        self.model_config = lila_config[cfg.model.encoder]
        self.model = TrainLILA(cfg.model, denorm=self.train_data.denorm, **self.model_config).cuda()

        run_root = cfg.runtime.root or os.path.join("runs", "train")
        run_name = cfg.runtime.name or "debug"
        self.logdir = os.path.join(run_root, run_name)
        os.makedirs(self.logdir, exist_ok=True)

        self.writer_train = tensorboard.SummaryWriter(os.path.join(self.logdir, "train"))
        self.writer_val = tensorboard.SummaryWriter(os.path.join(self.logdir, "val"))
    ...
    def training_step(self, batch, batch_idx):
        _, losses = self.model.train_forward(batch)
        total_loss = losses["flow"] + self.cfg.model.w_edge * losses["edge"]
        losses["total_loss"] = total_loss

        self.log("total", total_loss.item(), prog_bar=True)
    ...
    def visualise(self, showroom):
        ...
        denorm = self.train_data.denorm
        fetch_image = lambda idx: denorm(batch["view1"] if idx == 0 else batch["view2"]).cpu()
        outs, _ = self.model.train_forward(batch)
        ...
        self.writer_val.add_image("vis", torchvision.utils.make_grid(concat_all), self.step)

        w = (0.5 + 1000.0 * outs["wopt"]).clamp(0, 1)
        self.writer_val.add_image("w", torchvision.utils.make_grid(w, nrow=1), self.step)
...
@hydra.main(version_base=None, config_path="configs", config_name="train_ytvos")
def main(cfg: DictConfig) -> None:
    ...
    trainer = L.Trainer(devices=1, accelerator=accelerator, max_epochs=50, limit_train_batches=2000)
