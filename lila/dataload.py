...
def rand_crop(scale_range, output_size):
    ...
    # sample grid
    grid = F.affine_grid(theta, (1, 1, output_size, output_size), align_corners=False)
...
class LazyLoad:
    ...
    def cache_index(self, index):
        if not index in self.cache:
            self.cache[index] = np.fromstring(open(self.filenames[index], "rb").read(), np.uint8)
...
class VideoDataset(BaseDataset):
    ...
    def __init__(self, root_dir, split, crop_range, aspect_range, filetype="jpg", max_num_obj=8, preload=False):
        super().__init__()
        ...
        self.tf_A = A.Compose([A.SmallestMaxSize(224), \
                               A.CenterCrop(224, 224), \
                               A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
                               A.ToTensorV2()])
        
        self.tf_A_val = A.Compose([A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
                                   A.ToTensorV2()])
    ...
    def __getitem__(self, idx):
        ...
        view1, tf1 = self.crop_w_tf(base_crops[0], self.crop_range, self.aspect_range)
        view2, tf2 = self.crop_w_tf(base_crops[1], self.crop_range, self.aspect_range)
...
class YTVOS(BaseDataset):
    ...
    def __init__(self, root_dir, split, crop_range, aspect_range, filetype="jpg", T=2, mask_ratio=0.5):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        view1, tf1 = self.crop_w_tf(base_crops[0], self.crop_range, [1.0, 1.0], with_flip=True)
...
class SegDataset(BaseDataset):
    ...
    fine_to_coarse = {0: 9, 1: 11, 2: 11, 3: 11, 4: 11, 5: 11, 6: 11, 7: 11, 8: 11, 9: 8, 10: 8, 11: 8, 12: 8,
                      ...
                      177: 26, 178: 26, 179: 19, 180: 19, 181: 24, 255: 27}
    ...
    def __init__(self, root_dir, split, filelist, preload=False, val=False):
        ...
        for image_fn, mask_fn in zip(image_fns, mask_fns):
            assert Path(image_fn).stem == Path(mask_fn).stem, \
        ...
        self.masks = LazyLoad(mask_fns, preload, self.bytes2mask, np.array(LUT, dtype=np.int64))
        
        if val:
            self.tf = A.Compose([A.SmallestMaxSize(224), 
                                 A.CenterCrop(224, 224), 
                                 A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
                                 ToTensorV2()])
        else:
    ...
    def __getitem__(self, idx):
        ...
        return augmented['image'], augmented['mask'], torch.zeros_like(augmented['image'][:1])
...
class ZShotSegDataset(BaseDataset):
    ...
    def __init__(self, root_dir, split, filelist, preload=False, val=False):
        ...
        img_ids = [os.path.splitext(os.path.basename(fn))[0] for fn in filenames_data]
        ...
        if not val: # training
            ...
            self.seen_ids = [idx for idx in range(self.num_classes()) \
...
class NormalsDataset(BaseDataset):
    ...
    def __init__(self, root_dir, split, output_size=(224, 224)):
        ...
        self.depth_data = pickle.load(open(f"{root_dir}/nyu_{split}_depth.pkl", "rb"))
        self.image_data = pickle.load(open(f"{root_dir}/nyu_{split}_image.pkl", "rb"))
        self.norml_data = pickle.load(open(f"{root_dir}/nyu_{split}_normal.pkl", "rb"))

        self.output_size = output_size
        self.mode = split.lower()
        self.mult = 10 if split == "train" else 1

        self.image_normalization = T.Normalize(
        ...
        if self.mode == 'train':
            self.spatial_transform = T.Compose([
                T.RandomResizedCrop(self.output_size, scale=(0.5, 1.0), antialias=True),
                RandomHFlipWithNormals(p=0.5), 
            ])
            
            self.image_only_transform = T.Compose([
                T.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.2),
                self.image_normalization,
            ])
        else:
            self.spatial_transform = T.Compose([
                T.Resize(int(self.output_size[0] * 1.1), antialias=True),
    ...
    def __getitem__(self, idx):
        ...
        image, depth, norml = self.image_data[idx], self.depth_data[idx], self.norml_data[idx]

        image = F_v2.to_dtype(F_v2.to_image(image), dtype=torch.float32, scale=True)
        depth = F_v2.to_dtype(F_v2.to_image(depth), dtype=torch.float32, scale=False)
        norml = F_v2.to_dtype(F_v2.to_image(norml), dtype=torch.float32, scale=False)
...
class DepthDataset(BaseDataset):
    ...
    def __init__(self, root_dir, split, preload=False):
        ...
        self.tf = A.Compose([A.CenterCrop(224, 224), 
                             A.ToFloat(), ToTensorV2()], \
                                additional_targets={"depth": "image", "normal": "image"})
        self.tf_norm = T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ...
    def __getitem__(self, idx):
        ...
        # image, task labels, depth
        return self.tf_norm(augmented['image']), augmented['depth'], augmented['normal']
