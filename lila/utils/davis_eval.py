...
class DavisEvaluate:
    ...
    def add_sequence(self, gt_masks, res_masks):
        
        all_gt_masks = davis_masks(gt_masks)
        all_res_masks = davis_masks(res_masks)
        
        # no clear reason why drop last (from original DAVIS code)
    ...
    def dict(self):
        ...
        return {"JF": (np.mean(J["M"]) + np.mean(F["M"])) / 2.,
                "Jm": np.mean(J["M"]), "Jr": np.mean(J["R"]), "Jd": np.mean(J["D"]),
                "Fm": np.mean(F["M"]), "Fr": np.mean(F["R"]), "Fd": np.mean(F["D"])}
    
    def print_metrics(self):
        J, F = self.metrics_res['J'], self.metrics_res['F']
        # Generate dataframe for the general results
        print("J&F-Mean: {:4.3f}".format((np.mean(J["M"]) + np.mean(F["M"])) / 2.))
        print("  J-Mean: {:4.3f}".format(np.mean(J["M"])))
        print("J-Recall: {:4.3f}".format(np.mean(J["R"])))
        print(" J-Decay: {:4.3f}".format(np.mean(J["D"])))
        print("  F-Mean: {:4.3f}".format(np.mean(F["M"])))
        print("F-Recall: {:4.3f}".format(np.mean(F["R"])))
        print(" F-Decay: {:4.3f}".format(np.mean(F["D"])))
        


def davis_masks(masks):
    # [T,H,W] mask indices
